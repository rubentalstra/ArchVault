---
title: Reverse Proxy
description: Configure Traefik, Nginx, or Caddy as a reverse proxy for ArchVault.
sidebar:
  order: 3
---
...
:::tip
**Don't know which to pick?** Traefik is the recommended option — it integrates natively with Docker, auto-discovers services via labels, and handles TLS automatically. Nginx is the most widely deployed. Caddy is the easiest standalone option.
:::
...
<Tabs>
<TabItem label="Traefik (recommended)" icon="seti:default">
...
The production Compose file includes a Traefik service as an optional [profile](https://docs.docker.com/compose/how-tos/profiles/). This is the fastest way to get HTTPS running — no config files to create or copy.
...
:::tip
When using the Traefik profile, port 3000 is still exposed on the host by default. For extra security, you can remove the host port binding by overriding the app service with an additional Compose file or by editing `compose.prod.yaml` to remove the `ports` section from `app`.
:::
...
<TabItem label="Caddy" icon="seti:default">
...
Add Caddy as a service in your `compose.prod.yaml`:
...
services:
  ...
  app:
    # ... existing app config ...
    ports: []  # Remove host port binding — Caddy handles it
    networks:
      - archvault
