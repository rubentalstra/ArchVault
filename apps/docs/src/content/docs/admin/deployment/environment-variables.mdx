...
| `DOMAIN` | _(empty)_ | Domain for the Traefik host rule (e.g. `archvault.example.com`). Required when using the built-in Traefik Compose profile. |
| `ACME_EMAIL` | _(empty)_ | Email for Let's Encrypt certificate notifications. Required when using the built-in Traefik Compose profile. |
